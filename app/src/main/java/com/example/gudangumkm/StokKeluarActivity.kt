        ...
        lifecycleScope.launch {
            when (val result = barangRepository.getAll(userId)) {
                is Result.Success -> {
                    ...
                    val adapter = ArrayAdapter(this@StokKeluarActivity, android.R.layout.simple_dropdown_item_1line, barangNames)
                ...
                is Result.Error -> {
                    Toast.makeText(this@StokKeluarActivity, "Error: ${result.exception?.message}", Toast.LENGTH_SHORT).show()
                }
                is Result.Loading -> { }
            }
        }
    ...
    private fun setupListeners() {
        ...
        btnSimpan.setOnClickListener {
            ...
            if (jumlah > selectedBarang!!.stok) {
                etJumlah.error = getString(R.string.msg_stok_tidak_cukup)
                Toast.makeText(this, getString(R.string.msg_stok_tidak_cukup), Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            btnSimpan.isEnabled = false
            lifecycleScope.launch {
                // Reduce stock using Firestore transaction
                when (val result = barangRepository.kurangiStok(userId, selectedBarang!!.id, jumlah)) {
                    is Result.Success -> {
                        if (result.data) {
                            ...
                            when (val transaksiResult = transaksiRepository.insert(userId, transaksi)) {
                                is Result.Success -> {
                                    Toast.makeText(this@StokKeluarActivity, getString(R.string.msg_simpan_sukses), Toast.LENGTH_SHORT).show()
                                    finish()
                                }
                                is Result.Error -> {
                                    Toast.makeText(this@StokKeluarActivity, "Error: ${transaksiResult.exception?.message}", Toast.LENGTH_SHORT).show()
                                    btnSimpan.isEnabled = true
                                }
                                is Result.Loading -> { }
                            }
                        } else {
                            Toast.makeText(this@StokKeluarActivity, getString(R.string.msg_stok_tidak_cukup), Toast.LENGTH_SHORT).show()
                            btnSimpan.isEnabled = true
                        }
                    }
                    is Result.Error -> {
